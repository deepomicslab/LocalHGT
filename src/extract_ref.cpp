...
const unsigned char least_depth = 255;
const int k = 32;
...
long slide_window(unsigned char* record_ref_hit, int ref_len, long ref_index, long extract_ref_len, ofstream & interval_file, float hit_ratio, float perfect_hit_ratio){ //find density hits regions
    int frag_index = 0;
    int window = 500;
    ...
    for (int j = 0; j < ref_len; j++){
        ...
        // cout <<(int)ref_depth[j] << " " << peak_hit[j]<<endl;
        // find peak
...
void read_index(bool* coder, int* base, int k, char* comple, string index_name, string interval_name, short *choose_coder, float hit_ratio, float perfect_hit_ratio){
    ...
    while (!index_file.eof()){
        ...
        while (n < buffer_size){
            ...
            // cout << n<< "\t"<< buffer_size <<"\t" <<kmer_index  <<"\t" << (int)kmer_count_table[kmer_index]<<endl;
            if (kmer_index == 0){
                record_ref_hit[j] = 0;
            }
            else{
                record_ref_hit[j] = (int)kmer_count_table[kmer_index];
            }
            j += 1;
