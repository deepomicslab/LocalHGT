...
class Micro_homo():
    ...
    def ana_HGT_bkp(self):
        ...
        for bkp_key in bkp_key_list:
            ...
            if bkp.cross_split_reads < 10:
                continue

            self.for_each_bkp(bkp)
            index += 1
            if index > mic.sam_number:
                break
    ...
    def for_each_bkp(self, bkp):
        ...
        if len(from_seq) == len(to_seq):
            ...
            from_seq = from_seq[:self.cutoff] + "|" + from_seq[self.cutoff:]
            to_seq = to_seq[:self.cutoff] + "|" + to_seq[self.cutoff:]
            print ("genome_1", bkp.from_ref, bkp.from_bkp, bkp.from_strand, from_seq, file = fout, sep="\t")
            print ("genome_2", bkp.to_ref, bkp.to_bkp, bkp.to_strand, to_seq, file = fout, sep="\t")
            print ('', file = fout)
...
if __name__ == "__main__":
    ...
    seq_output = "/mnt/d/HGT/time_lines/hgt_ins_junction_segments.txt"
    fout = open(seq_output, 'w')
