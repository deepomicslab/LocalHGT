...
then map the Nanopore reads onto the reconstructed recipient genome.
"""
...
if __name__ == "__main__":
    ...
    ###################### in server ss620a #######################
    database = "/mnt/delta_WS_1/wangshuai/02.HGT/detection/reference/UHGG_reference.formate.fna"  # UHGG v1 reference
    workdir = "/mnt/delta_WS_1/wangshuai/02.HGT/detection/Hybrid/"
    meta_data = "//mnt/delta_WS_1/wangshuai/02.HGT/detection/Hybrid/SRP366030.csv.txt"  # the metadata of the used cohort with both short and long reads available
    data_pair = "/mnt/disk2_workspace/wangshuai/00.strain/32.BFB/SRP366030.ngs_tgs_pair.csv"  # records the Illumina and Nanopore sample pair that from a same individual, 
    #and can be generated by the function read_meta()
    design_file = "/mnt/delta_WS_1/wangshuai/02.HGT/detection/Hybrid//sample_design.tsv" # record corresponding individual ID for each sample
    result_dir = "/mnt/delta_WS_1/wangshuai/02.HGT/detection/Hybrid/hgt/result/"  # the folder to save the HGT breakpoints detected by LocalHGT
    identified_hgt = "/mnt/delta_WS_1/wangshuai/02.HGT/detection/Hybrid/match/SRP366030.identified_event.csv" # the file to save the complete HGT events detected by LocalHGT
    tgs_bam_dir = "/mnt/delta_WS_1/wangshuai/02.HGT/detection/Hybrid/nanopore_alignment/results/" # to record the nanopore alignment result
    verified_result = "/mnt/delta_WS_1/wangshuai/02.HGT/detection/Hybrid/match/SRP366030.verified_event.csv" # to record the validation result
