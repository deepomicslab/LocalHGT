    ...
    def update_cluster(self, xy_cluster, bkp):
        ...
        for cluster in xy_cluster:
            if bkp.ref1 == cluster.ref1 and bkp.ref2 == cluster.ref2 and bkp.direction == cluster.direction:
                if abs(bkp.ref1_positions[0] - cluster.ref1_positions[0]) < rlen and\
                    abs(bkp.ref2_positions[0] - cluster.ref2_positions[0]) < rlen:
                ...
                if abs(bkp.ref1_positions[0] - cluster.ref2_positions[0]) < rlen and\
                    abs(bkp.ref2_positions[0] - cluster.ref1_positions[0]) < rlen:
                    cluster.ref1_positions += bkp.ref2_positions
                    cluster.ref2_positions += bkp.ref1_positions
                    flag = True
                    # break
            # add more possible bkp loci
            # if bkp.ref1 == cluster.ref1 and bkp.direction == cluster.direction:
            #     if abs(bkp.ref1_positions[0] - cluster.ref1_positions[0]) < 2 * rlen:
            #         cluster.ref1_positions += bkp.ref1_positions
            # if bkp.ref2 == cluster.ref1 and bkp.direction == cluster.direction:
            #     if abs(bkp.ref2_positions[0] - cluster.ref1_positions[0]) < 2 * rlen:
            #         cluster.ref1_positions += bkp.ref2_positions
            # if bkp.ref1 == cluster.ref2 and bkp.direction == cluster.direction:
            #     if abs(bkp.ref1_positions[0] - cluster.ref2_positions[0]) < 2 * rlen:
            #         cluster.ref2_positions += bkp.ref1_positions
            # if bkp.ref2 == cluster.ref2 and bkp.direction == cluster.direction:
            #     if abs(bkp.ref2_positions[0] - cluster.ref2_positions[0]) < 2 * rlen:
            #         cluster.ref2_positions += bkp.ref2_positions
...
def add_support_split_reads(cluster, read_obj):
    ...
    for i in range(len(cluster.ref1_positions)):
        for j in range(len(cluster.ref2_positions)):
            if abs(read_obj.pos1 - cluster.ref1_positions[i]) < rlen and \
            abs(read_obj.pos2 - cluster.ref2_positions[j]) < rlen:   
...
def choose_acc_from_cluster(cluster):
    ...
    for readobj in cluster.support_reads:
        ...
        from_side = ''
        to_side = ''
        inte = 1 * rlen # search with a larger interval
